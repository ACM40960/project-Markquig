    ...
    for (i = 0; i < order*order; i++){
        for (j = 0; j < order*order; j++){
            if (fixed_points[i][j] == 1){   //if fixed cell
                ...
                if (i == 0 && j == 0){          //if first cell
                    CDF[position] = pow(cost[i][j] + 1, 2);
                }
                else {
                    CDF[position] = CDF[position-1] + pow(cost[i][j] + 1, 2);
                }
            }
            position++;
        }
...
int* Sudoku_SA::sample_sub_grid(int* pos_1){
    ...
    for (i = 0; i < order; i++){    //loop over rows
        ...
        for (j = 0; j < order; j++){    //loop over cols
            ...
            if (fixed_points[row][col] == 1 ||          //if fixed cell or provided
                (row == pos_1[0] && col == pos_1[1])){  // cell
                if (i == 0 && j == 0){
                    CDF[position] = 0;
                }
                else {
                ...
                if (i == 0 && j == 0){
                    CDF[position] = pow(cost[row][col] + 1, 2);
                }
                else {
                    CDF[position] = CDF[position-1] + pow(cost[row][col] + 1, 2);
                }
            }
            position++;
        }
